...
No automated tests are currently configured. There are no Jest, Vitest, Cypress, Playwright, Mocha, or Supertest scripts/dependencies in the project.
...
Backend behavior currently needs manual verification with a configured MongoDB database, Firebase account, OpenRouter key, and optionally Razorpay credentials.
...
The current repository ignores `server/.env`, `client/.env`, and `node_modules/` at the root. `client/.gitignore` also ignores `dist`, logs, editor files, and `.env`; `server/.gitignore` ignores `.env`. Review ignore rules before the first commit and rotate any secret that was ever committed or shared.

## Resume Description

### Two-line version

Built MockVerse, a MERN-based AI mock interview platform using React, Express, MongoDB, OpenRouter, Firebase Google authentication, browser speech APIs, and Razorpay credits. Implemented resume parsing with deterministic ATS scoring, timed answer evaluation, adaptive interview difficulty, analytics, and PDF reports.

### Three-bullet version

- Built a React/Vite and Express/Mongoose mock interview workflow with Firebase Google sign-in, JWT cookies, MongoDB persistence, HR/technical modes, timed questions, history, analytics, and PDF report generation.
- Integrated OpenRouter `openai/gpt-4o-mini` for structured resume parsing, role/resume-aware question generation, and JSON answer evaluation across confidence, communication, correctness, score, and feedback.
- Implemented a local ATS scoring engine and response-driven difficulty transitions, plus browser speech recognition/synthesis and Razorpay-backed interview credits.

## Questions I Should Be Able to Answer

### Basic project questions

1. **What problem does MockVerse solve?** It gives candidates an authenticated, role-based environment to practice timed interviews and receive AI feedback.
2. **Who is the target user?** Students and job seekers preparing for HR or technical interviews.
3. **What happens when an interview starts?** The client posts setup data to the server, which generates five questions, deducts 50 credits, stores the interview, and returns the questions.
4. **How many questions are generated?** The server asks OpenRouter for exactly five and keeps the first five non-empty response lines.
5. **What does a user receive after an interview?** Overall score, confidence/communication/correctness averages, per-question feedback, a score chart, difficulty distribution in the finish response, and a downloadable PDF.

### Architecture questions

6. **Why are the client and server separate?** The Vite React client handles UI and browser APIs, while Express protects secrets and owns database, AI, authentication, resume, and payment operations.
7. **How does the client communicate with the server?** Axios calls the local API base URL and sends cookies with `withCredentials: true`.
8. **Where are routes mounted?** `server/index.js` mounts `/api/auth`, `/api/user`, `/api/interview`, and `/api/payment`.
9. **Where is client state stored?** Redux Toolkit currently stores only `userData`; active interview state is local React state in `InterviewPage` and its child components.
10. **What is a current architectural limitation?** The client API URL and CORS origin are hardcoded for local development, and there is no deployment configuration.

### AI/LLM questions

11. **Which model is used?** OpenRouter is called with model `openai/gpt-4o-mini`.
12. **How is structured AI output handled?** Resume and answer prompts request strict JSON, then controllers call `JSON.parse` on the returned content.
13. **How are questions generated?** The server sends role, experience, mode, projects, skills, and resume text to OpenRouter and parses newline-separated questions.
14. **How are answers evaluated?** The server sends the question and answer to OpenRouter and stores confidence, communication, correctness, final score, and feedback.
15. **Does the project currently route questions by weak topic?** No. Topic fields exist in the schema, but the evaluation prompt does not request or persist topic weakness and no follow-up prompt injects it.
16. **Does the model receive a temperature setting?** No. Only model and messages are configured in the OpenRouter request.

### Backend questions

17. **How is adaptive difficulty implemented?** `submitAnswer` maps the current score and current difficulty to a next difficulty, then mutates the next embedded question before saving.
18. **How are credits enforced?** `generateQuestion` checks for at least 50 credits, deducts 50 after AI question generation, and saves the user.
19. **How are uploaded resumes handled?** Multer stores a file temporarily under `server/public`, `pdfjs-dist` extracts text, and the controller deletes the file after processing or in the error path.
20. **What happens when an answer times out?** The client submits elapsed time; the server records score zero and timeout feedback when it exceeds the question time limit.

### Database questions

21. **What are the main models?** `User`, `Interview`, embedded interview questions, and `Payment`.
22. **How are questions related to users?** An interview stores a `userId` reference to `User`; questions are embedded documents inside the interview.
23. **Are migrations used?** No migration or seed system exists; Mongoose model operations create/update documents.
24. **What is a production data concern?** The report lookup should enforce ownership against `req.userId`, and payment plan values should be validated on the server.

### Authentication questions

25. **How does Google login become a backend session?** Firebase authenticates the browser account, the client posts name/email to Express, and Express issues a seven-day JWT cookie.
26. **How are protected requests checked?** `isAuth` reads `req.cookies.token`, verifies it with `JWT_SECRET`, and attaches `req.userId`.
27. **What is the logout behavior?** The client calls `/api/auth/logout`, and the server clears the `token` cookie.

### Voice/API questions

28. **Is voice processed by the backend?** No. The browser converts speech to text with `webkitSpeechRecognition`; only text is sent to the backend.
29. **How does the AI speak?** The browser uses `SpeechSynthesisUtterance`; no external voice service is configured.
30. **How are payments verified?** The server computes an HMAC SHA-256 signature using `RAZORPAY_KEY_SECRET`, compares it with Razorpay's signature, marks the payment paid, and increments user credits.

### Deployment and difficult follow-ups

31. **How would you deploy this project?** Build the Vite client, host it statically, run Express as a Node service, configure environment variables, update the API URL/CORS origin, enable secure cookies, and configure Atlas/Firebase/Razorpay domains.
32. **Why might MongoDB connection fail?** Invalid credentials, an unencoded password, an incorrect Atlas SRV hostname, a stopped cluster, DNS failure, or a missing IP allowlist entry.
33. **What happens if OpenRouter returns malformed JSON?** `JSON.parse` throws and the controller returns a 500 response; stronger schema validation and retry/fallback handling would improve resilience.
34. **How would you implement topic-aware follow-ups?** Extend the answer prompt to return `topicWeakness`, persist it in the current question, add a `topicWeakness` field to the session context, and generate each next question with difficulty plus that topic. This is not implemented in the current repository.
35. **How would you implement adaptation tracing?** Add an `adaptationTrace` array to the interview schema and append question ID, previous difficulty, score, next difficulty, and topic weakness during answer submission. This is also not implemented currently.
36. **What would you test first?** Difficulty transition boundaries, invalid AI JSON, credit deduction, payment signature verification, protected routes, resume cleanup, ownership checks, and browser fallback when speech recognition is unavailable.

## Verification Notes

This README was written against the repository's current source and package files. Verified items include the client/server scripts, local ports and CORS origin, route paths, model fields, environment variable references, OpenRouter model and URL, browser voice APIs, Razorpay integration, ATS calculator, and current ignore rules.

Items intentionally not presented as implemented are topic-aware question routing, adaptation trace storage, automated tests, and deployment configuration. Node.js minimum version and production hosting provider are not declared by the repository and therefore need confirmation.
